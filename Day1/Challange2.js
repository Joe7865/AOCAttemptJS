// Source of function: https://stackoverflow.com/a/1293163
...
let arrayFromData = CSVToArray(csvData, ""); 
let sumArray = [];
let flattenedArray = arrayFromData.flatMap(line => line);
let currElfCals = 0;
flattenedArray.forEach(
    (currentVal) => {
        if (currentVal === '') {
            sumArray.push(currElfCals);
            currElfCals = 0;
        }
        else {
            currElfCals += parseInt(currentVal);
        }
    }
);
